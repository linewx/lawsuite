...
public class InstrumentParser {
    private static Logger logger = LoggerFactory.getLogger(InstrumentParser.class);
    ...
    public Instrument parseContext(ParseContext context) {
        ...
        if (!this.type.getInstrumentType().equals(instrumentType)) {
            throw new InstrumentParserException(instrumentType, InstrumentParserException.ErrorCode.UNSUPPORTED_TYPE);
        }
        ...
        String reason = ReasonUtil.getReason(reasonResults.get(0));
    ...
    private void validateField(List<String> fieldValues, String fieldName, Boolean required, Integer maxNumber) {
        ...
        if (maxNumber != null) {
            if(fieldValues != null && fieldValues.size() > maxNumber) {
                throw new InstrumentParserException(maxNumber + " or more than "+ fieldName +" have been found: " + fieldValues.toString(), InstrumentParserException.ErrorCode.VALIDATION);
            }
        }
    }
